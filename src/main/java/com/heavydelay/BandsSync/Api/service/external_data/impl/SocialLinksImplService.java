...
public class SocialLinksImplService implements ISocialLinks{
    ...
    @Override
    public SocialLinksResponseDto updateSocialLinksForBand(Band band, SocialLinksRequestDto dto) {
        SocialLinks socialUpdate = socialRepository.findById(band.getSocialLinks().getIdSocial()).orElseThrow(
            () -> new ResourceNotFoundException("The Social links not found")
        );
